...
const Table = (props) => {
  ...
  useEffect(() => {
    ...
    const updatedTables = initialTables.map((table) => {
      const bookedTimeSlots = JSON.parse(sessionStorage.getItem(`table${table.number}_bookedTimeSlots`)) || [];
      const filteredTimeSlots = timeslots.filter(slot => !bookedTimeSlots.includes(slot));
      ...
      const updatedTables = tables.map(table =>
        table.number === tableNumber
          ? {
              ...table,
              reserved: true,
              availableTimeSlots: table.availableTimeSlots.filter(slot => slot !== timeSlot),
            }
          : table
      );
      setTables(updatedTables);

      const bookedTimeSlots = JSON.parse(sessionStorage.getItem(`table${tableNumber}_bookedTimeSlots`)) || [];
      sessionStorage.setItem(`table${tableNumber}_bookedTimeSlots`, JSON.stringify([...bookedTimeSlots, timeSlot]));
